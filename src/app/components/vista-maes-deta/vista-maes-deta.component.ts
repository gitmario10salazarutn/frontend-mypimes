...
export class VistaMaesDetaComponent implements OnInit {
  ...
  public pro_cedula_ruc!: number
  public fcom_id!: number
  Producto: any
  pro_nombre = ""
    ...
    this.Detalle_comprasService.getDetallePorNameDetalle(this.fcom_id).subscribe(
      (detalle_compras: any) => {
        ...
        for (let i = 0; i < this.Detalle_comprass.length; i++) {
          ...
          this.Producto = this.productos.find(({ pro_id }) => pro_id === this.prod_id)
          this.Detalle_comprass[i].pro_nombre = this.Producto.pro_nombre
          console.log(this.Producto.pro_nombre)
          this.Detalle_comprass[i].pro_imagen = this.Producto.pro_imagen
          console.log(this.Producto.pro_imagen)
        }
      }, (error) => console.log(error)
    )
